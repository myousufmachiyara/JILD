...
class PurchaseInvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            Log::info('[Purchase] Invoice created', ['id' => $invoice->id]);
            return redirect()->route('purchase_invoices.index')
                ->with('success', 'Purchase Invoice created successfully.');

        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('[Purchase] Store failed', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            Log::info('[Purchase] Invoice updated', ['id' => $invoice->id]);
            return redirect()->route('purchase_invoices.index')
                ->with('success', 'Purchase Invoice updated successfully.');

        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('[Purchase] Update failed', ['id' => $id, 'error' => $e->getMessage()]);
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error('[Purchase] Destroy failed', ['id' => $id, 'error' => $e->getMessage()]);
    ...
    {
        $invoice = PurchaseInvoice::with(['vendor', 'items.product', 'items.variation', 'items.measurementUnit'])
            ->findOrFail($id);
        ...
        $html .= '<tr><td colspan="5" align="right"><b>Sub Total</b></td><td align="right"><b>' . number_format($totalAmount, 2) . '</b></td></tr>';

        if ($invoice->convance_charges > 0) {
            $totalAmount += $invoice->convance_charges;
            $html .= '<tr><td colspan="5" align="right">Conveyance</td><td align="right">' . number_format($invoice->convance_charges, 2) . '</td></tr>';
        }
        if ($invoice->labour_charges > 0) {
            $totalAmount += $invoice->labour_charges;
            $html .= '<tr><td colspan="5" align="right">Labour</td><td align="right">' . number_format($invoice->labour_charges, 2) . '</td></tr>';
        }
        if ($invoice->bill_discount > 0) {
            $totalAmount -= $invoice->bill_discount;
            $html .= '<tr><td colspan="5" align="right">Discount</td><td align="right">(' . number_format($invoice->bill_discount, 2) . ')</td></tr>';
        }

        $html .= '<tr style="background-color:#f5f5f5;"><td colspan="5" align="right"><b>Net Total</b></td><td align="right"><b>' . number_format($totalAmount, 2) . '</b></td></tr></table>';

        $pdf->writeHTML($html, true, false, true, false, '');

        if (!empty($invoice->remarks)) {
            $pdf->writeHTML('<b>Remarks:</b><br><span style="font-size:12px;">' . nl2br($invoice->remarks) . '</span>', true, false, true, false, '');
        }
    ...
    public function getInvoicesByItem($itemId)
    {
        $invoices = PurchaseInvoice::whereHas('items', fn($q) => $q->where('item_id', $itemId))
            ->with('vendor')->get(['id', 'vendor_id']);
        return response()->json($invoices->map(fn($inv) => ['id' => $inv->id, 'vendor' => $inv->vendor->name ?? '']));
    }
    ...
    public function getProductInvoices($productId)
    {
        try {
            $invoices = PurchaseInvoice::whereHas('items', fn($q) => $q->where('item_id', $productId))
                ->with(['items' => fn($q) => $q->where('item_id', $productId)])->get();
            return response()->json($invoices->map(function ($inv) {
                $item = $inv->items->first();
                return ['id' => $inv->id, 'number' => $inv->id, 'rate' => $item?->price ?? 0];
            }));
        } catch (\Exception $e) {
    ...
    private function postPurchaseEntries(PurchaseInvoice $invoice): void
    {
        ...
        $this->syncVoucherEntries(
            ...
            [
                ['dr' => '104001', 'cr_id' => $invoice->vendor_id, 'amount' => $itemsTotal, 'remarks' => 'Goods received'],
                ['dr' => '502001', 'cr_id' => $invoice->vendor_id, 'amount' => $conveyance,  'remarks' => 'Conveyance charges'],
                ['dr' => '502002', 'cr_id' => $invoice->vendor_id, 'amount' => $labour,      'remarks' => 'Labour charges'],
                ['dr_id' => $invoice->vendor_id, 'cr' => '402001', 'amount' => $discount,    'remarks' => 'Purchase discount'],
            ]
        );
