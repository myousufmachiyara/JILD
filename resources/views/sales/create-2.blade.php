...
@section('title', 'Create Sale Invoice 2')
...
<script>
  ...
  function loadVariations(row, productId, preselectVariationId = null) {
    ...
    $.get(`/product/${productId}/variations2`, function (data) {
      console.log(data.variation);
      let options = '<option value="">Select Variation</option>';

      (data.variation || []).forEach(v => {
        // Assuming only one attribute per variation (you can adapt if multiple)
        let attr = v.attributes && v.attributes[0];
        let attributeName = attr?.attribute?.name ?? '';
        let attributeValue = attr?.value ?? '';

        options += `<option value="${v.id}" data-value="${attributeValue}">${v.sku}</option>`;
      });
  ...
  function calcRowTotal(row) {
    ...
    // Get attribute value from selected variation option
    const attributeValue = parseFloat(row.find('.variation-select option:selected').data('value')) || 1;

    // Apply discount to price
    const discountedPrice = price - (price * discPercent / 100);

    // New calculation based on attribute value
    const totalQtyPart = qty * attributeValue;
    const totalPricePart = discountedPrice * attributeValue;
    const rowTotal = totalQtyPart + totalPricePart;
